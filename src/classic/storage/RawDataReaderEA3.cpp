...
RawDataReaderEA3::RawDataReaderEA3(IndexPtr index)
        : index(index),
          dbr_type(0),
          dbr_count(0),
          type_changed(false),
...
RawDataReaderEA3::~RawDataReaderEA3() {
...
const RawValue::Data* 
RawDataReaderEA3::find(const std::string& channel_name,
                       const epicsTime *start)
{
#   ifdef DEBUG_DATAREADER
    printf("- RawDataReader(%s)::find(%s)\n",
           index->getFilename().c_str(), channel_name.c_str());
#   endif
...
const RawValue::Data *RawDataReaderEA3::next()
{
    if (!header)
        throw GenericException(__FILE__, __LINE__,
...
// Read next sample, the one to which val_idx points.
const RawValue::Data *RawDataReaderEA3::nextFromDatablock()
{    
    ...
    read(dbr_type, dbr_count, raw_value_size, data, header->datafile, offset);
...
void RawDataReaderEA3::getHeader(const string& basename, FileOffset offset)
{
...
const RawValue::Data *RawDataReaderEA3::findSample(const epicsTime &start)
{
    LOG_ASSERT(header);
    LOG_ASSERT(data); 
#ifdef DEBUG_DATAREADER
...
void  RawDataReaderEA3::read(DbrType type, DbrCount count, size_t size, 
			       RawValue::Data *value,
